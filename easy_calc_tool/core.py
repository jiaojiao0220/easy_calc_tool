...
class EasyCalc:
    ...
    def _grouped_statistics(
        self, df: pd.DataFrame, columns: List, operations: List, group_by: List
    ) -> Dict:
    ...
    def _date_calc(self, df: Optional[pd.DataFrame] = None, **kwargs) -> Dict:
        ...
        if not result:
            raise InvalidParameterError(
                "Need at least one of: (start,end), (date,days), birth_date"
            )
    ...
    def _find_similar_columns(self, df: pd.DataFrame, keyword: str) -> List[str]:
        ...
        if matches:
            return matches
        raise InvalidParameterError(
            f"Column '{keyword}' not found. Available: {df.columns.tolist()}"
        )
    ...
    def _generate_summary(
        self, result: Dict, columns: List, operations: List, group_by: Any
    ) -> str:
