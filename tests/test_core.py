...
class TestEasyCalcInitialization:
	...
	def test_available_tools(self, calc):
		...
		expected_tools = [
			"arithmetic", "statistics", "aggregate", "filter_calc",
			"timeseries", "unit_convert", "date_calc", "describe",
			"correlation", "pivot"
		]
...
class TestStatistics:
	...
	def test_statistics_with_min_max(self, calc, sample_dataframe):
		...
		result = calc.calculate("statistics", sample_dataframe,
		                        columns="value", operations=["min", "max"])
	...
	def test_statistics_with_median(self, calc, sample_dataframe):
		...
		result = calc.calculate("statistics", sample_dataframe,
		                        columns="value", operations=["median"])
	...
	def test_statistics_grouped_multiple_columns(self, calc, sample_dataframe):
		...
		result = calc.calculate("statistics", sample_dataframe,
		                        columns=["value", "score"],
		                        operations=["mean"],
		                        group_by="category")  # 注意这里是 "category"
	...
	def test_statistics_natural_language_operations(self, calc, sample_dataframe):
		...
		result = calc.calculate("statistics", sample_dataframe,
		                        columns="value", operations="总和和平均")
...
class TestAggregate:
	...
	def test_aggregate_multiple_operations(self, calc, sample_data_list):
		...
		result = calc.calculate("aggregate", sample_data_list,
		                        columns=["sales"], operations=["sum", "mean"])
...
class TestFilterCalc:
	...
	def test_filter_basic(self, calc, sample_data_list):
		"""Test basic filter."""
		result = calc.calculate("filter_calc", sample_data_list,
		                        condition="sales > 200")
...
class TestTimeseries:
	...
	def test_timeseries_moving_average(self, calc, timeseries_dataframe):
		...
		result = calc.calculate("timeseries", timeseries_dataframe,
		                        operation="moving_average", window=5)
...
class TestUnitConvert:
	...
	def test_convert_km_to_mile(self, calc):
		...
		result = calc.calculate("unit_convert", None,
	...
	def test_convert_kg_to_lb(self, calc):
		...
		result = calc.calculate("unit_convert", None,
		                        value=100, from_unit="kg", to_unit="lb")
...
class TestDateCalc:
	...
	def test_age_calculation(self, calc):
		...
		result = calc.calculate("date_calc", None,
		                        birth_date="1990-05-15", reference_date="2024-01-01")
...
class TestCorrelation:
	...
	def test_correlation_basic(self, calc, correlation_dataframe):
		...
		result = calc.calculate("correlation", correlation_dataframe,
		                        method="pearson")
...
class TestPivot:
	...
	def test_pivot_basic(self, calc, sample_data_list):
		...
		result = calc.calculate("pivot", sample_data_list,
		                        index="product", values="sales", aggfunc="sum")
...
class TestCache:
	...
	def test_cache_enabled(self, calc_with_cache, sample_dataframe):
		...
		result1 = calc_with_cache.calculate("statistics", sample_dataframe,
		                                    columns="value", operations="sum")
		result2 = calc_with_cache.calculate("statistics", sample_dataframe,
		                                    columns="value", operations="sum")
	...
	def test_clear_cache(self, calc_with_cache, sample_dataframe):
		"""Test clearing cache."""
		calc_with_cache.calculate("statistics", sample_dataframe,
		                          columns="value", operations="sum")
...
class TestDataFormats:
	...
	def test_json_string_input(self, calc, sample_data_json):
		...
		result = calc.calculate("statistics", sample_data_json,
		                        columns="value", operations="sum")
		# 现在应该能正常解析
